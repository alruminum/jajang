...
describe('S09 RecordGuideScreen — 단일 흐름: 이어폰 chip 항상 노출 (impl/13 §5)', () => {
  ...
  it('songKey=brahms 진입 시 이어폰 chip 텍스트가 노출된다', () => {
    // GUIDE_ITEMS[2] 와 HeadphoneChip 이 동일 텍스트 → getAllByText 로 다중 매칭 회피
    const { getAllByText } = renderWith('brahms');
    expect(getAllByText('이어폰을 끼면 더 또렷하게 담겨요').length).toBeGreaterThan(0);
  });

  it('songKey=twinkle 진입 시에도 이어폰 chip 텍스트가 노출된다 (모드 조건 없음)', () => {
    const { getAllByText } = renderWith('twinkle');
    expect(getAllByText('이어폰을 끼면 더 또렷하게 담겨요').length).toBeGreaterThan(0);
  });

  it('미매핑 songKey 진입 시에도 이어폰 chip은 노출된다', () => {
    const { getAllByText } = renderWith('__no_lyrics__');
    expect(getAllByText('이어폰을 끼면 더 또렷하게 담겨요').length).toBeGreaterThan(0);
  });
});
