...
// jest.mock factory hoisting 안전: 외부 변수 참조 제거, factory 내 직접 jest.fn() 사용
...
const asyncStorageMockRef = (require('@react-native-async-storage/async-storage') as {
...
beforeEach(() => {
  ...
  asyncStorageMockRef.getItem.mockReset();
  asyncStorageMockRef.setItem.mockReset();
  asyncStorageMockRef.setItem.mockResolvedValue(undefined);
});
...
describe.skip('S09 RecordGuideScreen — 단일 흐름: 이어폰 chip 항상 노출 (impl/13 §5)', () => {
  beforeEach(() => {
    asyncStorageMockRef.getItem.mockResolvedValue('true'); // 모달 건너뜀
  });
...
describe('S09 RecordGuideScreen — 단일 흐름: 가사 박스 노출 (impl/13 §5)', () => {
  beforeEach(() => {
    asyncStorageMockRef.getItem.mockResolvedValue('true');
  });
...
describe.skip('S09 RecordGuideScreen — 이어폰 경고 모달: 첫 진입 시 노출 (impl/13 §5)', () => {
  beforeEach(() => {
    // dismissed 아님 → 모달 노출
    asyncStorageMockRef.getItem.mockResolvedValue(null);
  });
...
describe.skip('S09 RecordGuideScreen — 이어폰 모달: "이어폰 끼고 할게요" 탭 → 닫기만 (impl/13 §6)', () => {
  beforeEach(() => {
    asyncStorageMockRef.getItem.mockResolvedValue(null);
  });
  ...
  it('"이어폰 끼고 할게요" 탭 → AsyncStorage.setItem 호출 없음 (dismissed 저장 안 함)', async () => {
    ...
    await findByText('이어폰을 끼면 더 잘 담겨요');
    fireEvent.press(getByLabelText('돌아가기'));
    expect(asyncStorageMockRef.setItem).not.toHaveBeenCalled();
  });
...
describe.skip('S09 RecordGuideScreen — 이어폰 모달: "그래도 진행" 탭 (impl/13 §5)', () => {
  beforeEach(() => {
    asyncStorageMockRef.getItem.mockResolvedValue(null);
  });
  ...
  it('"그래도 진행" 탭 → AsyncStorage.setItem("@jajang:earphone_warning_dismissed", "true") 호출', async () => {
    ...
    expect(asyncStorageMockRef.setItem).toHaveBeenCalledWith(EARPHONE_WARNING_KEY, 'true');
  });
...
describe.skip('S09 RecordGuideScreen — 이어폰 모달: 이미 dismissed 시 미노출 (impl/13 §5, §8)', () => {
  beforeEach(() => {
    // 이미 dismissed
    asyncStorageMockRef.getItem.mockResolvedValue('true');
  });
