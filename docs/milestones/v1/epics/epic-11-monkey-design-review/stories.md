...
- [x] `npx mobile-qa-tour monkey --package com.jajang.app --events 100` 로 smoke 검증 (emulator 필요 — batch 04 에서 검증)
...
- [x] `npm run qa:init` 으로 jajang 디렉토리에 템플릿 첫 생성 검증 — 운영자 smoke (emulator 필요)
- [x] `npm run qa:monkey` 첫 run — 1000 events / crash 0 / 리포트 생성 확인 — 운영자 smoke (emulator 필요)
- [x] `npm run qa:tour` 첫 run — 6 화면 (S11 제외) screenshot + 휴리스틱 표 생성 — 운영자 smoke (emulator 필요)
...
- [x] config schema 에 `pencil` 블록 추가 (zod) + 선택 필드 (Story 2 task 02 단계에서 zod 정의 완료, 본 task 는 fixture 정합)
- [x] tour runner — pencil enabled 일 때 per-screen 리포트에 `<!-- pencil ref slot: nodeIds=[...] -->` 주석 + 실제 png 경로 placeholder 작성
- [x] jajang `qa.config.json` 에 S10 매핑 추가 (`["llTp1", "r97aM"]`)
- [x] `docs/qa/README.md` SOP 업데이트
- [x] 후속 epic 후보 등록 — Story 5 의 system-design.md §8 / backlog.md 추가로 통합
...
- [x] `backlog.md` 에픽 목록 표에 Epic 11 행 추가 (기존 등재 확인)
- [x] `backlog.md` 체크리스트 마지막에 Epic 11 항목 추가 (기존 등재 확인) + 후속 epic 후보 7건 추가
- [x] `CLAUDE.md` 문서 목록 갱신 (해당되는 경우 — epic 경로 노출 구조 X 라 skip)
- [x] 후속 epic 후보 명시 — `system-design.md §8` 표에 backlog 컬럼 추가
