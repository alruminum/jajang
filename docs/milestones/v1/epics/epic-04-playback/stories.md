...
- [x] 재생 화면 UI: 곡명, 재생/일시정지 버튼, 볼륨 슬라이더
- [x] 재생 상태 시각화 (재생 중 애니메이션 또는 파형)
- [x] 볼륨 슬라이더 → 즉시 반영
- [x] 무료 플랜: 화면 하단 배너 광고 영역 (F10 연계)
- [x] Premium/Trial: 광고 영역 없음
...
- [x] react-native-track-player `RepeatMode.Queue` 설정 (단일 master.mp3 반복)
- [x] 최대 10시간 연속 재생 (클라이언트 loop, 서버 재요청 없음)
- [x] 10시간 경과 시 페이드아웃 후 자동 종료 + 화면 알림
- [x] 로컬 캐시 파일 기반 재생 (스트리밍 아님)
- [x] PlayerSlice.currentSessionId 로 현재 재생 세션 추적 (구 currentTrackId 대체)
...
- [x] react-native-track-player 백그라운드 오디오 모드 설정
- [x] iOS: Background Modes — Audio 선언
- [x] Android: Foreground Service + 알림 채널 설정
- [x] Premium/Trial 유저: 화면 잠금 후에도 재생 유지
- [x] 무료 유저 (트라이얼 아님): 화면 잠금 시 재생 일시정지 + 업그레이드 팝업 (앱 포그라운드 복귀 시)
- [x] Rewarded Ad 언락 유저: 자정까지 백그라운드 허용 (F11 연계)
...
- [x] 재생 화면 타이머 아이콘 탭 → 옵션 시트
- [x] 옵션: 30분 / 1시간 / 2시간 / 6시간 / 10시간
- [x] 타이머 설정 후 화면 내 잔여 시간 표시 ("2시간 후 종료")
- [x] 타이머 종료 1분 전: 로컬 푸시 알림 (알림 허용 시)
- [x] 종료 시: 10초 페이드아웃 후 재생 완전 종료
- [x] 타이머 취소: 타이머 아이콘 탭 → "타이머 끄기" 옵션
- [x] 재생 수동 종료 시 타이머 초기화
...
- [x] Now Playing 정보 설정: 앱명 + 곡명 + (선택) 앨범 아트
- [x] iOS: MPNowPlayingInfoCenter 연동
- [x] Android: MediaSession 연동
- [x] 잠금화면 미디어 카드: 재생/일시정지 버튼 동작
- [x] 이어폰 버튼 (재생/일시정지) 하드웨어 제어 연동
