...
- [x] 무료 유저 "생성 N/3" 횟수 표시 (S07 포함 — PRD §F4)
...
- [x] 다시 녹음 횟수 제한 없음
- [x] 추가 녹음 유도 문구 ("한 번 더 녹음하면 더 풍성해집니다") 항상 표시
- [x] 무료 3회 소진 시: "자장가 만들기" 버튼 비활성 + "무료 3회 소진 — 구독하면 무제한" 가이드 + "구독하기" CTA
...
- [x] 클라이언트: S07(자장가 선택) / **S09(녹음 가이드)** / S10(녹음) 화면에 "생성 N/3" 표시 (무료 유저만, S08 표시 항목 제거)
