...
# impl/02 — [Story 2 / #192] 서버: DSP 파이프라인 (DspService + Celery task + MockDspService)

**Epic**: 03 — DSP 음원 후처리 생성  
**커버 스토리**: Story 2 (DSP 서버 파이프라인). *주의*: 구 버전에 포함되어 있던 Story 4 (clip cleanup) 는 본 impl 에서 분리되어 impl/04 단독 책임. impl/02 의 `clip_cleanup_task` 코드 블록 (§6) 은 impl/04 의 `tasks/clip_cleanup.py` 로 이전 (engineer 단계 정합).  
