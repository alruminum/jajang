...
- [x] RevenueCat SDK 설정 (iOS/Android 앱스토어 연동)
- [x] 구독 상품 설정: **월간 (₩3,900/월) / 연간 (₩29,000/년)** — LTD(평생결제) 없음
- [x] 구독 화면 UI: 플랜 카드 + 혜택 목록 + 결제 버튼
- [x] 혜택 표시: 광고 제거 + 백그라운드 무제한 + 오프라인 다운로드
- [x] 결제 완료 → entitlement 즉시 부여 → 광고 제거 + 기능 해제
- [x] 구독 복원 버튼 (기기 변경 시)
- [x] 구독 취소 링크 (앱스토어/플레이스토어 이동)
- [x] Apple IAP 강제 (앱스토어 규정 준수, 외부 결제 미노출)
...
- [x] Google Mobile Ads SDK 설정 (AdMob 계정, 앱 ID)
- [x] 재생 화면 하단 배너 광고 컴포넌트
- [x] 무료 플랜 유저에게만 노출, Premium/Trial 유저 숨김
- [x] 광고 로드 실패 시 영역 collapse (빈 공간 없음)
- [x] COPPA 설정: 아동 대상 광고 비활성화 (tagForChildDirectedTreatment = false)
- [x] GDPR: EU 유저 동의 처리 (UMP SDK 연동)
...
- [x] 업그레이드 팝업에 "광고 보고 오늘 밤 무료로 쓰기" 버튼 (트라이얼 유저에게는 미노출)
- [x] Rewarded Ad 로드 + 노출 (AdMob Rewarded)
- [x] 광고 시청 완료 검증 → 자정까지 백그라운드 재생 언락
- [x] 언락 상태 로컬 저장 (자정 초기화)
- [x] **월 7회 제한** (캘린더 월 기준 리셋) — 하루 1회 아님
- [x] 월 7회 소진 시 "이번 달은 이미 모두 사용했어요" 안내
- [x] 광고 로드 실패 시 "광고를 불러오지 못했어요" 안내
- [x] 트라이얼 유저: 팝업 진입 시 Rewarded Ad 버튼 미노출 (이미 Premium 상태)
...
- [x] Premium entitlement 확인 후 다운로드 버튼 노출
- [x] 음원 로컬 저장 (앱 전용 디렉토리)
- [x] 다운로드 상태 표시 (다운로드 중 / 완료)
- [x] 오프라인 재생 가능 배지 표시
- [x] 무료 유저 다운로드 시도 → 구독 유도 팝업
...
- [x] 업그레이드 팝업 트리거 상황 정의:
  - 무료 유저가 화면 잠금 시 (백그라운드 제한)
  - 무료 유저가 오프라인 다운로드 시도 시
  - 트라이얼 만료 시
  - 무료 유저 AI 생성 횟수 3회 소진 시 (횟수 소진 메시지 포함)
- [x] 팝업 UI: 혜택 요약 + "구독하기" CTA + "광고 보기" (Rewarded Ad, 트라이얼 중 미노출)
- [x] 구독 화면: 설정 메뉴 + 팝업 CTA에서 진입 가능
- [x] 팝업 닫기: X 버튼 or 바깥 탭 (닫아도 제한은 유지)
...
| 스토리 | GitHub Issue |
|---|---|
| Epic | [#71](https://github.com/alruminum/jajang/issues/71) |
| Story 1 | [#72](https://github.com/alruminum/jajang/issues/72) |
| Story 2 | [#73](https://github.com/alruminum/jajang/issues/73) |
| Story 3 | [#74](https://github.com/alruminum/jajang/issues/74) |
| Story 4 | [#75](https://github.com/alruminum/jajang/issues/75) |
| Story 5 | [#76](https://github.com/alruminum/jajang/issues/76) |
