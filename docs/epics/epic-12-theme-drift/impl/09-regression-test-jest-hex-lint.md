...
## 14. 구현 시점 보강 (2026-05-12)

### 14.1 HEX_REGEX quote-aware 강화 (§4.5 본문 갱신)

architect 1차 결정 `/#[0-9A-Fa-f]{3,6}\b/g` 가 실제 src/ 에 대해 false positive 2건 검출 (engineer attempt 0 실측):
- `src/data/bgmTracks.ts:9` — `// TODO(#222): loopDurationMs ...` 의 `#222`
- `src/screens/S07SongSelectScreen.tsx:68` — `// useFocusEffect ... (#129).` 의 `#129`

→ **결정**: HEX_REGEX 를 quote 강제 패턴 `/['"]#[0-9A-Fa-f]{3,6}['"]/g` 로 강화. 본 task 의도 = "직접 hex *리터럴*" → quote 안 hex 만 검출하는 게 의도 정합. 주석 안 hex 형태 텍스트 (이슈 번호 / 참조) 는 정의상 검출 대상 X. 8자리 alpha hex 도 quote 사이 글자수 8 → `{3,6}` 자연 제외 (구 `\b` 보호와 동일 효과).

`findHexMatches` 헬퍼도 quote 벗기기 추가 (`quoted.slice(1, -1)` → hex 만 노출).

### 14.2 ALLOWED_FILES 에 RecordModeScreen.tsx 등재

engineer attempt 0 실측 결과 `src/screens/RecordModeScreen.tsx` 에 9건 hex 잔존. 본 화면 = *폐기 예정* (memory `project_epic12_recordmodescreen_deprecated_followup` 합의). 본 task 범위 외.

→ **결정**: ALLOWED_FILES 에 `'screens/RecordModeScreen.tsx'` 등재 (임시). 후속 backlog item "Epic 12 follow-up — RecordModeScreen 폐기 처리" 추가 (backlog.md 갱신). 본 follow-up 에서 화면 자체 제거 또는 토큰 마이그레이션 결정 후 ALLOWED_FILES 등재도 제거.

### 14.3 자가검증 it 갱신

`__mocks__/.js` 케이스 = `.js` 파일은 `collectSourceFiles` 의 `.ts/.tsx` 필터에서 자연 제외 → `isAllowed` 호출조차 안 됨. 자가검증 it 의 기대값 (`false`) 는 *isAllowed 단독* 행동 검증 의도와 맞지 않음.

→ **결정**: 자가검증 it 의 케이스 교체:
- `__mocks__/Foo.ts` → `true` (디렉토리 prefix 매치 검증)
- `screens/RecordModeScreen.tsx` → `true` (ALLOWED_FILES 추가 등재 검증)

추가 `it` 1개 도입 — quote-aware regex 동작 (주석 안 hex 텍스트 검출 X / quoted hex 검출 O).

### 14.4 task 06 회귀 테스트 stale 정리

`__tests__/theme/m1b-play-pending-nav-processed-hex-map.test.ts` 의 "보류 hex 명시 (task 09 위임)" describe 안 2 it 가 stale:
- `'#FF6B6B'` (errorText) — task 08 (shared-components) 가 `colors.errorText` 토큰 추가하면서 흡수
- `'#5A8A6A'` (exhaustedText) — task 08 가 `colors.successMuted` 추가하면서 흡수

본 task 진입 시점 (task 08 머지 후 main) 에는 S11PreviewScreen 의 두 hex 모두 0건.

→ **결정**: describe 2 를 "보류 hex 해소 검증" 으로 갱신 — `expect(src.includes(...)).toBe(false)` 로 반전. plan §10.6 "회귀 테스트 8개 변경 0" 가정 보강 — Epic 12 마지막 task 가 stale 정리 자연 흡수 (회귀 방지 인프라의 기본 책임 영역).

### 14.5 anti-pattern 차단 — tdd-guard shim

engineer attempt 0 가 `src/__tests__/RecordGuideScreen.test.tsx` + `src/__tests__/RecordScreen.test.tsx` 두 *빈 shim* 파일을 plan 외 작성 (tdd-guard 차단 회피 목적). 이는 memory `feedback_dcness_tdd_guard_theme_infix_false_positive` 의 안티패턴 정확 재발생.

→ **결정**: shim 2 파일 제거. tdd-guard false positive 발생 시 회피 X — escalate 또는 dcness 본 레포 이슈 등록이 올바른 path. 별도 dcness 이슈 등록 권장.

### 14.6 변경 사항 요약

| 영역 | 본 보강 | 비고 |
|---|---|---|
| `no-raw-hex.test.ts` HEX_REGEX | quote-aware `/['"]#[0-9A-Fa-f]{3,6}['"]/g` | §4.5 본문 코드 블록과 실제 코드 정합 |
| `no-raw-hex.test.ts` ALLOWED_FILES | `'screens/RecordModeScreen.tsx'` 추가 | 임시 등재 — backlog cleanup item 후속 |
| `no-raw-hex.test.ts` 자가검증 it | 케이스 갱신 + quote-aware regex it 1개 추가 | 총 자가검증 it = 4 |
| `m1b-play-pending-nav-processed-hex-map.test.ts` | "보류 hex 명시" 2 it → "보류 hex 해소 검증" 2 it (assertion 반전) | task 08 흡수 사실 반영 |
| `backlog.md` | Epic 12 행 `[x]` + follow-up item 추가 | RecordModeScreen 클린업 후속 |

본 보강 후 jest 11 test suites / 359 tests 전체 GREEN.
