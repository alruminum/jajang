...
## 메타
- 생성 모드: UX_FLOW (INCREMENTAL #133 패치)
- PRD: prd.md (v1.2.1, 2026-04-28)
- 생성일: 2026-04-25 / 최종 갱신: 2026-04-29
- 화면 수: 17 screens + 1 component (C06)
- v1.2.1 갱신 범위: S09(가사 미리보기 + 헤드폰 권장), S10(허밍 모드 BGM 30% + 가사 박스). challenge-response 박스 폐기.
...
| S09 | 녹음 가이드 | 모드별 예시 + 허밍 모드 가사 미리보기 + 헤드폰 권장 안내 | F2 | 3 |
| S10 | 녹음 | 파형 + 실시간 녹음 + 생성 횟수 + 허밍 모드 한정 BGM 30% + 가사 박스 | F2, F3, F4, F5 | 6 |
...
> 허밍 모드: 곡별 한국어 번안 가사 미리보기 박스(1절 4~6줄) + 헤드폰 권장 chip 노출.
> 쉬 모드: 가사 박스·헤드폰 chip 미노출 (모드별 예시 일러스트 + 가이드 항목만).
...
│  [허밍 모드]              │
│                          │
│  [🎧 헤드폰을 끼면        │  ← 헤드폰 권장 chip (세이지 그린 outline)
│      더 또렷하게 담겨요]  │
│                          │
│  [모드별 가이드 일러스트] │
│                          │
...
| 헤드폰 chip 탭 (허밍) | — | 비인터랙티브 (안내 텍스트). 자동 헤드폰 감지·강제 모달은 V2 |

#### 상태
| 상태 | 조건 | 표시 |
|------|------|------|
| 기본 — 허밍 모드 | 가사 자산 준비됨 | 가이드 + 헤드폰 chip + 곡별 가사 박스 |
| 가사 미준비 — 허밍 모드 (fallback) | 선택 곡 가사 자산 없음 | 가사 박스 숨김 + "허밍해 주세요" 안내 (헤드폰 chip은 유지) |
| 기본 — 쉬 모드 | — | 가이드만, 가사 박스·헤드폰 chip 미노출 |
...
> 허밍 모드: 카운트다운 종료와 동시에 선택 곡 BGM 30% 재생 시작 + 가사 박스 노출. 녹음 종료(자동/수동) 시 BGM 즉시 정지.
> 쉬 모드: BGM·가사 박스 미노출 (기존 동작 유지).
...
| 카운트다운 종료 (허밍 모드) | 선택 곡 BGM 30% 재생 시작 + 가사 박스 표시 | 파형·가사·BGM 동시 노출 |
| 카운트다운 종료 (쉬 모드) | — | 파형만 노출 (BGM·가사 박스 미노출) |
| 60초 경과 | 자동 종료 + (허밍) BGM 즉시 정지 | S11 이동 |
| ⏹ 탭 (30초+) | 녹음 중지 + (허밍) BGM 즉시 정지 | S11 이동 |
| ⏹ 탭 (30초 미만) | (허밍) BGM 즉시 정지 | "조금 더 녹음해주세요" 다이얼로그 → 이어서/재시작 |
| 다시 녹음 진입 (S11→S10) | 카운트다운 재시작 후 BGM 동일 곡 처음부터 재생 | — |
| ✕ 취소 탭 | (허밍) BGM 즉시 정지 | 취소 확인 팝업 → 확인: S08 이동 |

#### 상태
| 상태 | 조건 | 표시 |
|------|------|------|
| 카운트다운 | 3초 | 3→2→1 대형 숫자 (BGM·가사 박스 미노출) |
| 녹음 중 — 허밍 (미달) | 0~29초 + BGM 정상 + 가사 자산 OK | 가사 박스 + BGM 30% chip + 파형 + "30초 채워주세요" |
| 녹음 중 — 허밍 (충족) | 30초~ | 위 + ⏹ 활성 (안내 숨김) |
| 녹음 중 — 허밍 (BGM 로드 실패) | BGM URL 에러/타임아웃 | 가사 박스 유지 + 상단 토스트 "음악 없이 녹음할게요" + BGM chip 미노출 |
| 녹음 중 — 허밍 (가사 미준비 fallback) | 선택 곡 가사 자산 없음 | 가사 박스 숨김 + "허밍해 주세요" 안내 + (BGM 정상 시) BGM chip 유지 |
| 녹음 중 — 쉬 모드 | — | 파형만 (BGM·가사 박스 미노출) |
| 30초 미만 종료 시도 | ⏹ 탭 (미달) | "조금 더 해주세요" 다이얼로그 + (허밍) BGM 정지 상태 유지 |
...
| S09 | 녹음 가이드 | SCREEN | P1 | 허밍 모드 가사 미리보기 박스 + 헤드폰 권장 chip / 쉬 모드는 가이드만 / 가사 미준비 fallback 1종 |
| S10 | 녹음 | SCREEN | P0 | 실시간 파형 + 허밍 모드 한정 BGM 30% chip + 가사 박스. BGM 로드 실패·가사 미준비 fallback 2종 |
